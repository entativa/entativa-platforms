...
async fn main() -> std::io::Result<()> {
    ...
    tracing::info!("🔐 Starting Socialink Messaging Service (Signal-level E2EE)...");
...
async fn health_check() -> HttpResponse {
    HttpResponse::Ok().json(serde_json::json!({
        "status": "healthy",
        "service": "Socialink Messaging Service",
        "version": "1.0.0",
...
async fn root() -> HttpResponse {
    HttpResponse::Ok().json(serde_json::json!({
        ...
        "documentation": "https://docs.socialink.com/messaging"
    }))
}
