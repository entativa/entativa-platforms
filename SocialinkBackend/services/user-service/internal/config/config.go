...
func LoadConfig() *Config {
	return &Config{
		...
		S3: S3Config{
			...
			BucketName:      getEnv("S3_BUCKET_NAME", "socialink-media"),
			Region:          getEnv("S3_REGION", "us-east-1"),
			CDNURL:          getEnv("S3_CDN_URL", ""),
			UsePathStyle:    getEnvAsBool("S3_USE_PATH_STYLE", true), // true for MinIO
		},
		Kafka: KafkaConfig{
...
func getEnvAsSlice(key string, defaultValue []string) []string {
	...
	return []string{valueStr} // Simplified for now
}
