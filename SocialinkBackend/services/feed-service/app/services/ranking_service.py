...
class RankingService:
    ...
    async def get_socialink_candidates(
        self, user_profile: UserProfile, limit: int = 600
    ) -> List[ContentFeatures]:
        """
        Get candidates for Socialink unified feed
        Prioritizes social graph (friends, following, groups, pages)
        """
        candidates = []
        
        # 1. Friends' content (highest priority - last 24h)
        if user_profile.friend_ids:
            friend_content = await self._get_content_by_users(
                user_profile.friend_ids, limit=250
            )
            candidates.extend(friend_content)
        
        # 2. Following content (users, pages, creators)
        if user_profile.following_ids:
            following_content = await self._get_content_by_users(
                user_profile.following_ids, limit=150
            )
            candidates.extend(following_content)
        
        # 3. Groups content
        # In production: Get user's groups and fetch content
        groups_content = await self._mock_get_content(
            filter_type="groups", limit=100
        )
        candidates.extend(groups_content)
        
        # 4. Interest-based content (pages, topics)
        ...
        # 5. Discovery (suggested content)
        discovery_content = await self._get_viral_content(limit=50)
        candidates.extend(discovery_content)
        ...
        logger.info(f"Retrieved {len(unique_candidates)} candidates for Socialink feed")
    ...
    async def _mock_get_content(self, filter_type: str, **kwargs) -> List[ContentFeatures]:
        ...
        for i in range(min(limit, 20)):
            mock_items.append(ContentFeatures(
                ...
                hashtags=[f"#{filter_type}", "#socialink"],
                topics=[f"topic_{i % 5}"],
