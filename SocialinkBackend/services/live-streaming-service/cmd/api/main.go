...
func main() {
	...
	if dbURL == "" {
		dbURL = "postgresql://postgres:postgres@localhost:5432/socialink_streaming?sslmode=disable"
	}
	...
	router.GET("/health", func(c *gin.Context) {
		c.JSON(200, gin.H{"status": "healthy", "service": "socialink-live-streaming"})
	})

	// Root
	router.GET("/", func(c *gin.Context) {
		c.JSON(200, gin.H{
			"service": "socialink-live-streaming-service",
			"version": "1.0.0",
			"features": []string{
