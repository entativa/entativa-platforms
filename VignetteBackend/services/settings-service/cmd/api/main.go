...
import (
	...
	"github.com/entativa/vignette/settings-service/internal/handler"
...
func main() {
	...
	if dbURL == "" {
		dbURL = "postgresql://postgres:postgres@localhost:5432/vignette_settings?sslmode=disable"
	}
	...
	router.GET("/health", func(c *gin.Context) {
		c.JSON(200, gin.H{"status": "healthy", "service": "vignette-settings-service"})
	})

	// Root
	router.GET("/", func(c *gin.Context) {
		c.JSON(200, gin.H{
			"service": "vignette-settings-service",
			"version": "1.0.0",
			"features": []string{
	...
	log.Printf("🚀 Vignette Settings Service starting on port %s", port)
