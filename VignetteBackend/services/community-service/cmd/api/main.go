...
import (
	...
	"github.com/entativa/vignette/community-service/internal/handler"
	"github.com/entativa/vignette/community-service/internal/repository"
...
func main() {
	...
	if dbURL == "" {
		dbURL = "postgresql://postgres:postgres@localhost:5432/vignette_community?sslmode=disable"
	}
	...
	router.GET("/health", func(c *gin.Context) {
		c.JSON(200, gin.H{"status": "healthy", "service": "vignette-community-service"})
	})

	// Root
	router.GET("/", func(c *gin.Context) {
		c.JSON(200, gin.H{
			"service": "vignette-community-service",
			"version": "1.0.0",
			"features": []string{
	...
	log.Printf("🚀 Vignette Community Service starting on port %s", port)
