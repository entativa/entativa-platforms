...
class FeedService:
    ...
    async def generate_feed(
        ...
        """
        Generate personalized Socialink feed
        Single unified feed: 70% friends, 20% interests, 10% discovery
        """
        logger.info(
            f"Generating unified feed for user {request.user_id}"
        )
        ...
        # Generate unified feed (friends-first)
        ranked_items = await self._generate_unified_feed(
        ...
        logger.info(
            f"Generated {len(paginated_items)} items for unified feed"
        )
    ...
    async def _get_candidates(
        ...
        """
        Get candidate content for Socialink unified feed
        Mix of friends, following, groups, pages, interests
        """
        # Get unified candidates (social-first)
        candidates = await self.ranking_service.get_socialink_candidates(
            user_profile, limit=600
        )
        
        return candidates
    
    async def _generate_unified_feed(
        self,
        ...
        """
        Generate Socialink unified feed
        70% friends, 20% interests, 10% discovery
        """
        logger.info(f"Ranking {len(candidates)} candidates for unified feed")
        
        ranked_items = self.rec_engine.rank_home_feed(
            user_profile, candidates, seen_ids
        )
        
        logger.info(f"Ranked {len(ranked_items)} items for unified feed")
        return ranked_items
